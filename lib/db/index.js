...
// In-memory cache for session persistence on Vercel
const cache = {
  users: null,
  medications: null,
  alerts: null
...
function getCacheKey(filePath) {
  ...
  if (filePath === MEDICATIONS_FILE) return 'medications';
  if (filePath === ALERTS_FILE) return 'alerts';
  return null;
}
...
function writeJSON(filePath, data) {
  ...
  try {
    ...
    console.warn(`Persistence Error (Vercel Read-Only): ${err.message}. Data preserved in session memory.`);
  }
}
...
export function createUser({ name, email, password, allergies = [] }) {
  ...
  if (existingUser) {
    return { id: existingUser.id, name: existingUser.name, email: existingUser.email, allergies: existingUser.allergies };
  }
...
export function updateUser(id, updates) {
  ...
  if (updates.age !== undefined) users[idx].age = parseFloat(updates.age) || null;
  if (updates.dateOfBirth !== undefined) users[idx].dateOfBirth = updates.dateOfBirth;
  if (updates.name !== undefined) users[idx].name = updates.name;
  if (updates.allergies !== undefined) users[idx].allergies = updates.allergies;
